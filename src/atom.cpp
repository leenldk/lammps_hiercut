...
void Atom::sort()
{
  ...
  int* numneigh_sort = force->pair->list->numneigh_sort;
  if (force->pair->reorder_neighbor == ON_ATOM_SORT && !numneigh_sort) {
    printf("reset nextsort to %d\n", update->ntimestep + 1);
    nextsort = update->ntimestep + 1;
  }
  ...
  if (force->pair->reorder_neighbor == ON_ATOM_SORT && numneigh_sort) {
    for (m = 0; m < nbins; m++) {
      ...
      if (m & 1) {
        std::sort(permute + n_prev, permute + n, [&](int a, int b) {
          return numneigh_sort[a] < numneigh_sort[b];
        });
      }
      else {
        std::sort(permute + n_prev, permute + n, [&](int a, int b) {
