...
class Pair : protected Pointers {
 ...
 public:
  ...
  // initialize p_fhcut and fhcut_value to invalid
  float p_fhcut = -1.0f;
  float fhcut_value = -1.0f;
  PRECISION_TYPE prec_type = DEFAULT_PREC;
  // method type id of kernel, default to -1, need to be explicitly specified when used
  int method_type = -1;
  // whether or not using relative coordinate for pair computation 
  bool use_relative_coord = false;
  // whether or not sparate special neighbors for pair computation 
  bool use_sep_sepcial = false;
  // whether or not reorder atoms according to neighbor size  
  NEIGH_REORDER_STRATEGY reorder_neighbor = NO_NEIGH_REORDER;
  // neighbor separation strategies for half2 calculations
  NEIGH_SEP_STRATEGY neigh_sep_strategy = NO_NEIGH_SEP;
  // bool use_half2 = false;
  // bool use_basic_fhcut = false;
  // bool use_two_end_neigh = false;
  // bool use_int2_neigh = false;

  double neighbor_build_time = 0.0;
  double neighbor_sort_time = 0.0;
  ...
  void summary_neighbor() {
    printf("pair::summary_neighbor:\n");
    printf("pair::neighbor_sort_time : %lf\n", neighbor_sort_time);
    printf("pair::neighbor_build_time : %lf\n", neighbor_build_time);
  }
