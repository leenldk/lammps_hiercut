...
int Input::execute_command()
{
  ...
  else if (mycmd == "fhcut") fhcut();
  else if (mycmd == "prec_type") prec_type();
  else if (mycmd == "pair_relative_coord") pair_relative_coord();
  else if (mycmd == "pair_sep_special") pair_sep_special();
  else if (mycmd == "reorder_neigh") reorder_neigh();
  else if (mycmd == "method_type") method_type();
...
/// options : USE_HALF2, USE_FHCUT, TWO_END_NEIGH, INT2_NEIGH
/// first arg must be p_fhcut value \in [0, 1]
/// USE_HALF2 : whether to use half2 for half precision part calcaulation
/// use neighbor separation strategies "basic_fhcut" and "two_end_neigh" require "use_half2" to be set first
/// "basic_fhcut" and "two_end_neigh" can not be used together
/// "use_int2_neigh" requires "two_end_neigh" be set first
///
/// basic_neigh_sep = USE_HALF2 + USE_FHCUT
/// two_end_neigh = USE_HALF2 + USE_FHCUT + TWO_END_NEIGH
/// two_end_neigh_int2 = USE_HALF2 + USE_FHCUT + TWO_END_NEIGH + INT2_NEIGH
void Input::fhcut()
{
  // printf("### processing cmd fhcut\n");
  if (narg < 2) error->all(FLERR,"Illegal fhcut command: expected 2 or more than 2 argument but found {}", narg);
  if (force->pair->prec_type != HFMIX_PREC) {
    error->all(FLERR,"fhcut command can only be used with hfmix precision type");
  }

  if (strcmp(arg[0], "ratio") == 0) {
    // use p_fhcut ratio to specify fhcut 
    force->pair->p_fhcut = utils::numeric(FLERR,arg[1],false,lmp);
    if (force->pair->p_fhcut < 0 || force->pair->p_fhcut > 1) {
      error->all(FLERR,"p_fhcut ratio must be in [0, 1]");
    }
    printf("set p_fhcut to: %f\n", force->pair->p_fhcut);
  }
  else if (strcmp(arg[0], "value") == 0) {
    // use fhcut radius value to specify fhcut
    force->pair->fhcut_value = utils::numeric(FLERR,arg[1],false,lmp);
    if (force->pair->fhcut_value < 0) {
      error->all(FLERR,"fhcut value must be non-negative");
    }
    printf("set fhcut_value to: %f\n", force->pair->fhcut_value);
  }
  else {
    error->all(FLERR,"Unknown fhcut option: {}", arg[1]);
  }
  if (narg == 3) {
    if (strcmp(arg[2], "basic_neigh_sep") == 0) {
      // basic two-pointer neighbor separation strategy
      force->pair->neigh_sep_strategy = BASIC_NEIGH_SEP;
      printf("set neigh_sep_strategy to BASIC_NEIGH_SEP\n");
    }
    else if (strcmp(arg[2], "basic_neigh_sep_rev") == 0) {
      // basic two-pointer neighbor separation strategy, put outer neighbor first
      force->pair->neigh_sep_strategy = BASIC_NEIGH_SEP_REV;
      printf("set neigh_sep_strategy to BASIC_NEIGH_SEP_REV\n");
    }
    else if (strcmp(arg[2], "basic_neigh_sep_opt") == 0) {
      // BASIC_NEIGH_SEP strategy, with optimized neighbor separation
      force->pair->neigh_sep_strategy = BASIC_NEIGH_SEP_OPT;
      printf("set neigh_sep_strategy to BASIC_NEIGH_SEP_OPT\n");
    }    
    else if (strcmp(arg[2], "two_end_neigh") == 0) {
      // birdirectional neighbor separation strategy
      force->pair->neigh_sep_strategy = TWO_END_NEIGH;
      printf("set neigh_sep_strategy to TWO_END_NEIGH\n");
    }
    else if (strcmp(arg[2], "two_end_neigh_int2") == 0) {
      // birdirectional neighbor separation strategy with int2 neighbor list
      force->pair->neigh_sep_strategy = TWO_END_NEIGH_INT2;
      printf("set neigh_sep_strategy to TWO_END_NEIGH_INT2\n");
    }
    else if (strcmp(arg[2], "multi_neigh_list") == 0) {
      // separate different precision neighbors into different neighbor list
      force->pair->neigh_sep_strategy = MULTI_NEIGH_LIST;
      printf("set neigh_sep_strategy to MULTI_NEIGH_LIST\n");
    }
    else {
      error->all(FLERR,"Unknown neigh_sep_strategy option: {}", arg[2]);
    }
  }
}

void Input::prec_type()
{
  if (narg != 1) error->all(FLERR,"Illegal prec_type command: expected 1 argument but found {}", narg);

  if (strcmp(arg[0],"default") == 0) {
    force->pair->prec_type = DEFAULT_PREC;
    printf("set prec_type to default precision\n");
  }
  else if (strcmp(arg[0],"double") == 0) {
    force->pair->prec_type = DOUBLE_PREC;
    printf("set prec_type to double precision\n");
  }
  else if (strcmp(arg[0],"float") == 0) {
    force->pair->prec_type = FLOAT_PREC;
    printf("set prec_type to float precision\n");
  }
  else if (strcmp(arg[0],"half") == 0) {
    force->pair->prec_type = HALF_PREC;
    printf("set prec_type to half precision\n");
  }
  else if (strcmp(arg[0],"hfmix") == 0) {
    force->pair->prec_type = HFMIX_PREC;
    printf("set prec_type to half-float mixed precision\n");
  }
  else if (strcmp(arg[0],"dfmix") == 0) {
    force->pair->prec_type = DFMIX_PREC;
    printf("set prec_type to double-float mixed precision\n");
  }
  else {
    error->all(FLERR,"Unknown precision type: {}", arg[0]);
  }
}

void Input::pair_relative_coord() {
  if (narg != 1) error->all(FLERR,"Illegal pair_relative_coord command: expected 1 argument but found {}", narg);

  force->pair->use_relative_coord = utils::logical(FLERR,arg[0],false,lmp);
  printf("set use_relative_coord to: %d\n", force->pair->use_relative_coord);
}

void Input::pair_sep_special() {
  if (narg != 1) error->all(FLERR,"Illegal pair_sep_special command: expected 1 argument but found {}", narg);

  force->pair->use_sep_sepcial = utils::logical(FLERR,arg[0],false,lmp);
  printf("set use_sep_sepcial to: %d\n", force->pair->use_sep_sepcial);
}

void Input::reorder_neigh() {
  if (narg != 1) error->all(FLERR,"Illegal reorder_neigh command: expected 1 argument but found {}", narg);

  if (strcmp(arg[0], "on_neigh_build") == 0) {
    force->pair->reorder_neighbor = ON_NEIGH_BUILD;
    printf("set reorder_neighbor to: ON_NEIGH_BUILD\n");
  }
  else if (strcmp(arg[0], "on_atom_sort") == 0) {
    force->pair->reorder_neighbor = ON_ATOM_SORT;
    printf("set reorder_neighbor to: ON_ATOM_SORT\n");
  }
  else {
    error->all(FLERR,"Unknown reorder_neighbor option: {}", arg[0]);
  }
  // force->pair->reorder_neighbor = utils::logical(FLERR,arg[0],false,lmp);
  // printf("set reorder_neighbor to: %d\n", force->pair->reorder_neighbor);
}

void Input::method_type() {
  if (narg != 1) error->all(FLERR,"Illegal method_type command: expected 1 argument but found {}", narg);
  force->pair->method_type = utils::numeric(FLERR,arg[0],false,lmp);
}
