...
class NPairKokkos : public NPair {
 ...
 private:
  ...
  typename AT::t_int_scalar d_new_maxneighs_outer;
  typename AT::t_int_scalar d_new_maxneighs_int2;
  typename AT::t_int_scalar d_new_maxneighs_special;
  HAT::t_int_scalar h_resize;
  HAT::t_int_scalar h_new_maxneighs;
  HAT::t_int_scalar h_new_maxneighs_outer;
  HAT::t_int_scalar h_new_maxneighs_int2;
  HAT::t_int_scalar h_new_maxneighs_special;
  ...
  DAT::tdual_int_1d_3 k_stencil_dir;
};
...
class NeighborKokkosExecute
{
 ...
 public:
  ...
  typename AT::t_int_1d_3 d_stencil_dir;

  // data from Atom class

  const typename AT::t_x_array_randomread x;
  const typename AT::t_x_array x_rel;
  const typename AT::t_x_array bin_base;
  const typename AT::t_int_1d fhcut_split;
  ...
  const X_FLOAT binsizex,binsizey,binsizez;
  ...
  typename AT::t_int_scalar new_maxneighs_outer;
  typename AT::t_int_scalar new_maxneighs_int2;
  typename AT::t_int_scalar new_maxneighs_special;
  typename ArrayTypes<LMPHostType>::t_int_scalar h_resize;
  typename ArrayTypes<LMPHostType>::t_int_scalar h_new_maxneighs;
  typename ArrayTypes<LMPHostType>::t_int_scalar h_new_maxneighs_outer;
  typename ArrayTypes<LMPHostType>::t_int_scalar h_new_maxneighs_int2;
  typename ArrayTypes<LMPHostType>::t_int_scalar h_new_maxneighs_special;
  ...
  NeighborKokkosExecute(
                        ...
                        const typename AT::t_int_1d_3 &_d_stencil_dir,
                        const int _nlocal,const int _nall,const int _neigh_transpose,
                        const typename AT::t_x_array_randomread &_x,
                        const typename AT::t_x_array &_x_rel,
                        const typename AT::t_x_array &_bin_base,
                        const typename AT::t_int_1d &_fhcut_split,
                        ...
                        const int &_molecular, const float &_p_fhcut, const float &_fhcut_rsq,
                        ...
                        const X_FLOAT &_binsizex,const X_FLOAT &_binsizey,const X_FLOAT &_binsizez,
                        ...
                        const typename AT::t_int_scalar _new_maxneighs_outer,
                        const typename ArrayTypes<LMPHostType>::t_int_scalar _h_new_maxneighs_outer,
                        const typename AT::t_int_scalar _new_maxneighs_int2,
                        const typename ArrayTypes<LMPHostType>::t_int_scalar _h_new_maxneighs_int2,
                        const typename AT::t_int_scalar _new_maxneighs_special,
                        const typename ArrayTypes<LMPHostType>::t_int_scalar _h_new_maxneighs_special):
    ...
    nstencil(_nstencil),d_stencil(_d_stencil),d_stencilxyz(_d_stencilxyz),d_stencil_dir(_d_stencil_dir),
    x(_x),x_rel(_x_rel),bin_base(_bin_base),fhcut_split(_fhcut_split),
    radius(_radius),type(_type),mask(_mask),molecule(_molecule),
    tag(_tag),special(_special),nspecial(_nspecial),molecular(_molecular),p_fhcut(_p_fhcut), fhcut_rsq(_fhcut_rsq),
    ...
    binsizex(_binsizex),binsizey(_binsizey),binsizez(_binsizez),
    ...
    new_maxneighs_outer(_new_maxneighs_outer),h_new_maxneighs_outer(_h_new_maxneighs_outer),
    new_maxneighs_int2(_new_maxneighs_int2),h_new_maxneighs_int2(_h_new_maxneighs_int2),
    new_maxneighs_special(_new_maxneighs_special),h_new_maxneighs_special(_h_new_maxneighs_special) {
    ...
    h_new_maxneighs_outer() = neigh_list.maxneighs_outer;
    h_new_maxneighs_int2() = neigh_list.maxneighs_int2;
    h_new_maxneighs_special() = neigh_list.maxneighs_special;
  };

  ~NeighborKokkosExecute() {neigh_list.copymode = 1;};

  template<int HalfNeigh, int Newton, int Tri, int USE_RELATIVE_COORD, int USE_SEP_SPECIAL, int REORDER_NEIGH, NEIGH_SEP_STRATEGY NEIGH_STG>
  KOKKOS_FUNCTION
  ...
  //void build_ReleativeX(const int &i) const;
  ...
  void bin_base_coord(X_FLOAT &x, X_FLOAT &y, X_FLOAT &z, const int &idx) const {
    int ix = idx % mbinx;
    int iy = (idx/mbinx) % mbiny;
    int iz = idx/(mbinx*mbiny);

    x = ix * binsizex + bboxlo[0];
    y = iy * binsizey + bboxlo[1];
    z = iz * binsizez + bboxlo[2];
  }
...
template<class DeviceType, int HALF_NEIGH, int NEWTON, int TRI, int USE_RELATIVE_COORD, int USE_SEP_SPECIAL, int REORDER_NEIGH, NEIGH_SEP_STRATEGY NEIGH_STG>
struct NPairKokkosBuildFunctor {
  ...
  void operator() (const int & i) const {
    c.template build_Item<HALF_NEIGH,NEWTON,TRI, USE_RELATIVE_COORD, USE_SEP_SPECIAL, REORDER_NEIGH, NEIGH_STG>(i);
  }
#ifdef LMP_KOKKOS_GPU
  LAMMPS_DEVICE_FUNCTION inline
  void operator() (typename Kokkos::TeamPolicy<DeviceType>::member_type dev) const {
    c.template build_ItemGPU<HALF_NEIGH,NEWTON,TRI, USE_RELATIVE_COORD, USE_SEP_SPECIAL, REORDER_NEIGH, NEIGH_STG>(dev, sharedsize);
  }
  size_t team_shmem_size(const int team_size) const { (void) team_size; return sharedsize; }
#endif
  // KOKKOS_INLINE_FUNCTION
  // void operator() (FunctorTags::ReleativeXTag, const Kokkos::Cuda::size_type i) const {
  //   c.build_ReleativeX((int)i);
  // }
};

template<int HALF_NEIGH, int NEWTON, int TRI, int USE_RELATIVE_COORD, int USE_SEP_SPECIAL, int REORDER_NEIGH, NEIGH_SEP_STRATEGY NEIGH_STG>
struct NPairKokkosBuildFunctor<LMPHostType,HALF_NEIGH,NEWTON,TRI, USE_RELATIVE_COORD, USE_SEP_SPECIAL, REORDER_NEIGH, NEIGH_STG> {
