...
// struct FunctorTags {
//   struct ReleativeXTag {};
// };

// whether or not using X coordinates relative to bin
// #define RELATIVE_COORD

// whether or not using half2 for half computation
// #define USE_HALF2

// whether or not enable cutoff between float and half
// #define USE_FHCUT
...
// whether or not store neighbors from two endings
// float precision from end to start, half precision from start to end
// conflict with USE_FHCUT
// #define TWO_END_NEIGH

#ifdef TWO_END_NEIGH
// store the neighbor of half2 into int2, and store the neighbor of float from front
#define INT2_NEIGH
#endif
...
struct __align__(8) AoS_half_xonly {
  __half x[3];
};

struct __align__(8) AoS_halfq {
  __half x[3];
  uint8_t type;
  uint8_t q_type;
};
...
struct __align__(8) Coul_LJ_entry {
  float coul_val;
  __half2 lj_val;
};
...
typedef Kokkos::DualView<int2**, LMPDeviceType::array_layout, LMPDeviceType> tdual_neighbors_2d_int2;
typedef tdual_neighbors_2d_int2::t_dev t_neighbors_2d_int2;
typedef tdual_neighbors_2d_int2::t_dev_const t_neighbors_2d_int2_const;
