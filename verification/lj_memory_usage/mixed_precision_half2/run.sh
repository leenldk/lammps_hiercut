...
CUDA_VISIBLE_DEVICES=2 OMP_PROC_BIND=spread OMP_PLACES=threads mpirun --bind-to core --map-by socket:PE=20 -np 1 ncu --set detailed --replay-mode application --import-source yes --kernel-name fhmix_expr_optimized --launch-skip 196 --launch-count 1 -o report $EXE -k on t 20 g 1 -sf kk -in $INPUT 2>&1 | tee $OUT_FILE
